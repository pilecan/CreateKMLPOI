...
public class BuildCVS {
	...
	static public void createCVS(ArrayList<Poi> pois, String title,String region, String cvsfile) {
		Writer writer = null;
		try {
	
			writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(cvsfile), "utf-8"));


			for (int i = 0; i < pois.size(); i++) {
			//	writer.write(buildCVS(i,title,region, pois.get(i)));
				System.out.println(buildCVS(i,title,region, pois.get(i)));
	...
	static public String buildCVS(int number, String title, String region,Poi poi) {
		
		String type = "POI";
		String url = (poi.getName()+" "+region).replaceAll(" ", "%20");
		
		url = google+url.replaceAll(",","%20");
		
/*		History,Osaka Temple,POI-1,34.68734392107492, 135.52581055074313,,,MSFS World Update 20 Japan,
		"Google : https://www.google.com/search?q=Hanau%20(Frankfurt)%20Germany \n test", Japan,,2000,,,		
*/		
		
		
		String cvsLine = type+",\""+poi.getName()+"\",POI"+number+","+poi.getLatx()+","+poi.getLony()+",,,"
		+title+","+url+","+region+",,50";
